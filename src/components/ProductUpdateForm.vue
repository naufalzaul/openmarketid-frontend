...
selectedTaxes.value = productStore?.product?.productTaxes;

const name = useProductField("name");

function useProductField(field) {
  return computed({
    get: () => productStore.product?.[field] ?? "",
    set: (val) => {
      if (productStore.product) {
        productStore.product[field] = val;
      }
    },
  });
}
...
const handleSubmit = async (event) => {
  ...
  try {
    const response = await updateProduct({
      ...
      availableQuantity: parseInt(values.productStock),
    });
            ...
            <div class="sm:col-span-2">
              ...
              <div class="mt-2">
                <input
                  ...
                  v-model="name"
                  :placeholder="
                    !productStore.product?.name ? 'Enter product name' : ''
                  "
            ...
            <div class="sm:col-span-2">
              ...
              <div class="mt-2">
                <input
                  ...
                  :placeholder="
                    !productStore.product.price ? 'Enter product price' : ''
                  "
            ...
            <div class="sm:col-span-2">
              ...
              <div class="mt-2">
                <input
                  ...
                  name="productStock"
                  id="product-stock"
                  v-model="productStore.product.availableQuantity"
                  :placeholder="
                    !productStore.product.availableQuantity
                      ? 'Enter product stock'
                      : ''
                  "
                  class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm"
